  ...
  render() {
    return (
     ...
     <HeaderComponent routeName={this.props.navigation.state.routeName} nav={this.props.navigation}/>
        ...
        <Score score={this.state.score} points={this.state.points} />
        {/* <Button
    ...
    if(cards[index].is_open == false && selected_pairs.indexOf(cards[index].name) === -1){
      ...
      if(current_selection.length == 2){
        if(current_selection[0].name == current_selection[1].name){
          score += 1;
          if(score===12){
            Alert.alert("You have successfully earned 50 points!");
          ...
          AsyncStorage.getItem('POINTS').then((value) => {
            ...
            this.setState({points: value+50});

            AsyncStorage.setItem('POINTS', this.state.points.toString());
          });

            // let key = parseInt(AsyncStorage.getItem('POINTS'));
            // AsyncStorage.setItem('POINTS', (key + 50).toString());


            // AsyncStorage.setItem('POINTS',this.state.points);
            ...
            // );
            // this.setState({
            //   points : parseInt(this.state.points)+50
            // });
